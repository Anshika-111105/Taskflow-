...
export default function RegisterPage() {
  ...
  const onSubmit = async (d: FormData) => {
    setError('')
    try {
      const { data } = await authService.register(d.name, d.email, d.password)
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">Start managing tasks smarter</p>
        </div>
        <div className="card p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input className="input" type="password" placeholder="Min 6 characters" {...register('password', { required: 'Required', minLength: { value: 6, message: 'Min 6 characters' } })} />
